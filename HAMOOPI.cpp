...
/*******************************************************************************
	TO DO LIST: 
	Aqui vai a minha lista de Prioridades (n�o est� em ordem):
	...
	7- Escrever a documenta��o do jogo
	8- Revisar / otimizar todo o c�digo
*******************************************************************************/
...
int EndRoundT=500; //tempo de espera at� reinciar os rounds
...
{
	set_uformat(U_ASCII); //permite usar acentua��o no jogo (diacr�ticos)
	allegro_init(); install_timer(); install_keyboard(); install_mouse(); set_color_depth(32);
	//set_gfx_mode() � declarado logo abaixo, apos carregar dados do SETUP.INI
	...
	//define se resolucao � fullscreen
	...
	while (sair==0)
	{
		...
		if (DonationScreen==1){
			...
			if (strcmp (IDIOMA,"BR")==0){ 
				...
				textprintf_centre_ex(bufferx, font_19, 380, 55+10, makecol(000,000,000), -1, "doa��o de qualquer quantia de" );  
		...
		if (ApresentacaoMode==1){
			...
			if (strcmp (IDIOMA,"BR")==0){ 
				...
				textprintf_centre_ex(bufferx, font_30, 322, 227+30*1, makecol(000,000,000), -1, "VERSUS" );
				textprintf_centre_ex(bufferx, font_30, 322, 227+30*2, makecol(000,000,000), -1, "EDITOR" );
				textprintf_centre_ex(bufferx, font_30, 322, 227+30*3, makecol(000,000,000), -1, "OP��ES" );
				textprintf_centre_ex(bufferx, font_30, 322, 227+30*4, makecol(000,000,000), -1, "SAIR" );
				textprintf_centre_ex(bufferx, font_30, 320, 225+30*0, makecol(155,155,155), -1, "ARCADE" );
				textprintf_centre_ex(bufferx, font_30, 320, 225+30*1, makecol(255,255,255), -1, "VERSUS" );
				textprintf_centre_ex(bufferx, font_30, 320, 225+30*2, makecol(255,255,255), -1, "EDITOR" );
				textprintf_centre_ex(bufferx, font_30, 320, 225+30*3, makecol(255,255,255), -1, "OP��ES" );
				textprintf_centre_ex(bufferx, font_30, 320, 225+30*4, makecol(255,010,005), -1, "SAIR" );
				if (menu_op==1) textprintf_centre_ex(bufferx, font_30, 320, 225+30*0, makecol(255,170,000), -1, "ARCADE" );
				if (menu_op==2) textprintf_centre_ex(bufferx, font_30, 320, 225+30*1, makecol(255,170,000), -1, "VERSUS" );
				if (menu_op==3) textprintf_centre_ex(bufferx, font_30, 320, 225+30*2, makecol(255,170,000), -1, "EDITOR" );
				if (menu_op==4) textprintf_centre_ex(bufferx, font_30, 320, 225+30*3, makecol(255,170,000), -1, "OP��ES" );
		...
		if (OptionsMode==1){
			...
			if (strcmp (IDIOMA,"BR")==0){ 
				...
				textprintf_ex(bufferx, font_20, 192, 82+(espacamento* 1), makecol(025,025,025), -1, "Rounds: %i", RoundTotal); 
				textprintf_ex(bufferx, font_20, 192, 82+(espacamento* 2), makecol(025,025,025), -1, "Rel�gio: %i", RoundTime/60); 
				...
				textprintf_ex(bufferx, font_20, 190, 80+(espacamento* 1), makecol(255,255,255), -1, "Rounds: %i", RoundTotal); 
				textprintf_ex(bufferx, font_20, 190, 80+(espacamento* 2), makecol(255,255,255), -1, "Rel�gio: %i", RoundTime/60); 
				...
				if (options_op== 2) textprintf_ex(bufferx, font_20, 190, 80+(espacamento* 1), makecol(255,170,000), -1, "Rounds: %i", RoundTotal);
				if (options_op== 3) textprintf_ex(bufferx, font_20, 190, 80+(espacamento* 2), makecol(255,170,000), -1, "Rel�gio: %i", RoundTime/60); 
		...
		if (SelectCharMode==1) {
			...
			//atribui IDs de Navega��o / Carrega primeira imagem BIG ao inicializar
			...
			if (timermenus>=15 && NumPersonagensEscolhidos<2 && ( 
				...
				for(int ind=0; ind<=9; ind++){
					...
					P[1].Special_Inputs_c[ind][1]  = get_config_int(str, "c1" , 0); //0 � neutro
					...
					P[1].Special_Inputs_b[ind][1]  = get_config_int(str, "b1" , 0)*-1; //0 � neutro
			...
			if (timermenus>=15 && NumPersonagensEscolhidos<2 && ( 
				...
				for(int ind=0; ind<=9; ind++){
					...
					if(ind==8) { sprintf(str, "780"); }
					if(ind==9) { sprintf(str, "790"); }
					P[2].Special_Inputs_c[ind][1]  = get_config_int(str, "c1" , 0); //0 � neutro
					...
					P[2].Special_Inputs_b[ind][1]  = get_config_int(str, "b1" , 0)*-1; //0 � neutro
	...
	if (GamePlayMode==1){		
		...
		//timer_rounds � um contador de frames crescente utilizado em animacoes
		timer_rounds++; 
		if (timer_rounds>=250) { desabilita_players=0; } else { RoundTime++; }
		
		//final de round //RoundTime � um contador de frames decrescente utilizado pelo relogio da luta
		...
		//Um pre requisito para ativar os movimentos � ter as imagens correspondentes na pasta do personagem.
							...
							for(int indx=0; indx<=9; indx++){
								...
								if( especial_ativado==indx ){
									if( P[ind].TotalDeFramesMov[num]>-1 && P[ind].State!=num  ){
										...
										// MomentoDoP...==1 significa que o Player esta de P�
										// solucao temporaria
										int validacao=0; 
										...
										if(validacao==1){
											...
											//primeiro preciso saber quantos inputs tem o especial em quest�o
											int totaldeinputs=0;
		...
		//Dois corpos nao ocupam o mesmo lugar no espa�o :)
		if(contatofisico==1){
		...
		for(int ind=LIMITEINF; ind<=LIMITESUP; ind++){
			...
			//lembrando que o atuador do primeiro frame � carregado no LOAD_PLAYER
		...
		if(P[1].x<MeioDaTela) { VScreen_FatorZoom=abs((P[1].x-80)-(P[2].x+80)); } //no inicio do round o valor � 320 }
		else                  { VScreen_FatorZoom=abs((P[2].x-80)-(P[1].x+80)); } //no inicio do round o valor � 320 }
		...
		//define a prioridade de desenho, quem ataca � desenhado por cima do oponente
		for(int ind=1;ind<=2;ind++){
			if(
				/*P1_HitBox_tot>0 || */
		...
		for(int ind=1; ind<=2; ind++){
			...
			if(P[ind].QtdeMagias>0){
				...
				//precisa otimizar, pois nao � reamente necessario fazer  blit toda hora, apenas na hora de mudar o frame de animacao
			...
			if(Fireball[ind].Ativa==2){
				...
				//� necessario armazenar os valores em vetores, matriz bidimensional, para otimizar o codigo 
	...
	if (EditMode==1) {
		...
		if (key_Mouse_L_status==1 && mouse_x>88 && mouse_x<165 && mouse_y>120 && mouse_y<150) //Avan�ar States L
		{ MovPossiveisIndex++; if (MovPossiveis[MovPossiveisIndex]==0) { MovPossiveisIndex--; } ED_CarregarDados=1; }
		if (key_Mouse_R_status==1 && mouse_x>88 && mouse_x<165 && mouse_y>120 && mouse_y<150) //Avan�ar States R
...
//efeito anima��o de Introducao (AnimIntro)
if(ApresentacaoMode==1){
	if (AnimIntroTimer!=-1){
		for (int ind=0;ind<16;ind++){
...
void Aplicar_HIT(){
	...
	if ( colisaoxP1==1 || colisaoxP1Fireball==1 )
	{
		...
		//colisao contra P1 de uma fireball lan�ada pelo P2
		if(colisaoxP1Fireball==1){
	...
	if ( colisaoxP2==1 || colisaoxP2Fireball==1 )
	{
		...
		//colisao contra P2 de uma fireball lan�ada pelo P1
		if(colisaoxP2Fireball==1){
	...
	//Chacoalha os personagens ap�s o Hit, e os desloca
	if (P1HitPause>0 || P2HitPause>0) 
	{
...
void New_Fireball(int Player){
	...
	if(P[Player].State==700 && P[Player].TotalDeFramesMov[701]>-1 && P[Player].QtdeMagias==0){
		...
		if(Player==1){
			//tem q verificar a for�a de variacao tb (DMG), vou implementar depois.
...
void ED_load_charini(){
	...
	//salva o tempo de animacao 6 (se o tempo do frame for vazio, usa 6, que � o padrao)
	if(ED_State<700){
